...
region_table <- data.frame(region = getRegionTypes(dsa),
                num_region = c(getNRegions(dsa, "promoters"), 
                               getNRegions(dsa, "distal_regions")))

write.table(region_table, row.names = FALSE, col.names = FALSE,
            file = here("res", "atac-seq", "01_qc", 
...
write.table(num_fragments, row.names = TRUE, col.names = FALSE,
            file = here("res", "atac-seq", "01_qc", 
...
saveRDS(all_tn5_sites, here("res", "large_files", 
...
for (i in seq_along(sample_list)) {
  plot <- plotInsertSizeDistribution(dsa, sample_list[i]) + ggtitle(cell_lines[i])
