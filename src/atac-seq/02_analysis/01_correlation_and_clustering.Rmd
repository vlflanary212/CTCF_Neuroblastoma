...
promoter_fpkm <- read_csv(here("dat", "atac-seq", "combined", "counts",
                             "promoters_fpkm_counts.csv"))
```

```{r load metadata}
hncc <- read_csv(here("doc", "atac-seq", "GSE108517", "SraRunTable.txt"))
nb <- read_csv(here("doc", "atac-seq", "GSE138293", "SraRunTable.txt"))
```

# Wrangle the data
```{r combine metadata into a single table}
metadata <- bind_rows(hncc, nb)
```
...
Heatmap(distal_corr, column_title = "Pearson Correlation of Distal and 
...
```{r plot clusters}
pdf(here("res", "atac-seq", "02_eda", "hierarchical_clustering_of_distal_peaks.pdf"))
