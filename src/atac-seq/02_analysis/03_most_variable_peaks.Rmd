...
distal_fpkm <- read_csv(here("dat", "atac-seq", "combined", "counts",
                             "distal_fpkm_counts_by_cluster.csv")) %>% as.data.frame()
```

```{r load metadata}
metadata <- read_csv(here("doc", "atac-seq", "combined", "relevant_metadata.csv"))
```
...
cluster_col <- brewer.pal(6, "YlOrBr")
anno_pal <- list("Cluster" = setNames(cluster_col, cluster),
                 "MYCN_Status" = c("Amplified" = "darkred", "Non-amplified" = "azure4"))
